...
def build_model(params):
    ...
    with slim.arg_scope([slim.ops.conv2d, slim.ops.fc], is_training=is_training):
        model = slim.ops.conv2d(_x, 20, [5, 5], stddev=1/math.sqrt(5*5*params['input_shape'][2]), padding='VALID', scope='conv1')
        model = slim.ops.max_pool(model, [2, 2], padding='VALID', scope='pool1')
        model = slim.ops.conv2d(model, 50, [5, 5], stddev=1/math.sqrt(5*5*20), padding='VALID', scope='conv2')
        model = slim.ops.max_pool(model, [2, 2], padding='VALID', scope='pool2')
        model = slim.ops.flatten(model)
        model = slim.ops.fc(model, 500, stddev=1/math.sqrt(4*4*50), scope='fc1')
        model = tf.nn.relu(model, name='relu')
        model = slim.ops.dropout(model, 0.75, scope='do1')
        model = slim.ops.fc(model, params['nclasses'], stddev=1/math.sqrt(500), activation=None, scope='fc2')
